<template>
  <div>
    <h2>
      <span id="gateway-page-heading" v-text="t$('gateway.title')"></span>
      <button class="btn btn-primary float-right" @click="refresh()" :disabled="updatingRoutes">
        <font-awesome-icon icon="sync"></font-awesome-icon> <span v-text="t$('gateway.refresh.button')"></span>
      </button>
    </h2>
    <h3 v-text="t$('gateway.routes.title')"></h3>
    <div class="table-responsive">
      <table class="table table-striped">
        <thead>
          <tr>
            <th v-text="t$('gateway.routes.url')"></th>
            <th v-text="t$('gateway.routes.service')"></th>
            <th v-text="t$('gateway.routes.servers')"></th>
          </tr>
        </thead>
        <tbody>
          <tr v-for="route of gatewayRoutes" :key="route.serviceId">
            ...
            <td>
              <div v-if="route.serviceInstances.length === 0" class="label label-danger" v-text="t$('gateway.routes.error')"></div>
              <div class="table-responsive">
